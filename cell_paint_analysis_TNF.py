...
OUTPUT_RSLT_DIR  = EXPERIMENT_DIR / "rslt_TNF"
...
CONTROL_LABEL = 'TNF_1_4' #ctrl_rows["treatment_full"].values[0]
...
if not FINAL_OUTPUT.exists() or BOOL_NEW:
    ...
    final_profiles[keep_keys].to_csv(FINAL_OUTPUT)
...
meta_cols    = ["ImageNumber", "Cells_Number_Object_Number", "Name", "treatment_full",
                "Date", "concentration_numeric", "alpha"]
...
if DO_PCA or DO_TSNE or DO_UMAP:
    ...
    meta_cols_sample = ["ImageNumber", "Image_Count_Cells", "Cells_Number_Object_Number",
                        "Name", "treatment_full", "Date", "concentration_numeric", "alpha"]
        ...
        for t in treatments:
            ...
            for conc in metadata[metadata["treatment_full"] == t]["concentration_numeric"].unique():
                ...
                plt.scatter(X_tsne[mask, 0], X_tsne[mask, 1],
                            s=SCATTER_SIZE, label=t, color=clr, alpha=alpha)
